    ...
    send(req, res) {
        return this.store.create(req)
            .then((File) => {
                ...
                const baseUrl = req.headers['x-forwarded-path'] || req.baseUrl || '';
                const url = this.store.relativeLocation ? `${req.baseUrl || ''}${this.store.path}/${File.id}` : `//${host}${baseUrl}${this.store.path}/${File.id}`;
                console.log(req);

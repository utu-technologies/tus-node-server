    ...
    write(req, file_id, offset) {
        return new Promise((resolve, reject) => {
            ...
            return req.pipe(stream).on('finish', () => {
                ...
                // if uploadLengthDeferred = true on the client side,
                // upload-length header is set only on the last PATCH
                // which is why config.upload_length doesn't work properly

                if (config && parseInt(req.headers['upload-length'], 10) === offset) {
